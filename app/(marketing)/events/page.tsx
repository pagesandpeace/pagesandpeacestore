      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        {events.length === 0 ? (
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {events.map((event) => {
              const eventSeries = event.series_id ? seriesById.get(event.series_id) : null;
              return (
                <article key={event.id} className="overflow-hidden rounded-2xl border bg-white shadow-sm">
                  ...
                  <div className="space-y-3 p-6">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      ...
                      {eventSeries ? (
                        <Link
                          href={`/events/${eventSeries.slug}`}
                          className="inline-flex rounded-md border border-black/15 px-4 py-2 font-semibold text-neutral-900 hover:bg-neutral-50"
                        >
                          View series
                        </Link>
                      ) : null}
