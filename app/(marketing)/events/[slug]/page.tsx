...
export default async function EventOrSeriesPage({params}:PageProps){
 ...
 const jsonLd={"@context":"https://schema.org","@type":"Event",name:event.title,description:event.short_description??event.description,startDate:event.starts_at,eventStatus:past?"https://schema.org/EventCompleted":"https://schema.org/EventScheduled",eventAttendanceMode:"https://schema.org/OfflineEventAttendanceMode",location:{"@type":"Place",name:"Pages & Peace",address:{"@type":"PostalAddress",streetAddress:"8 Eva Building, Kings Avenue",addressLocality:"Rossington, Doncaster",postalCode:"DN11 0PF",addressCountry:"GB"}},image:event.image_url?[event.image_url]:undefined,url:`${SITE}/events/${event.slug}`,offers:event.ticket_types.map(t=>({"@type":"Offer",price:(t.price_pence/100).toFixed(2),priceCurrency:"GBP",availability:soldOut?"https://schema.org/SoldOut":"https://schema.org/InStock",url:`${SITE}/events/${event.slug}`}))};
