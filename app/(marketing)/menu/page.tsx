...
export default async function MenuPage() {
  ...
  const [{ data: sections }, { data: categories }, { data: items }] = await Promise.all([
    supabase.from("menu_sections").select("id, name, slug, position").eq("display_mode", "menu_tab").eq("is_visible", true).order("position"),
  ...
  const sectionTree = sectionRows.map((section) => ({
    ...section,
  ...
  return <main className="min-h-screen bg-[#FAF6F1] px-6 py-12 font-[Montserrat] text-[#111]">
    ...
    <div className="mt-16 text-center"><Link href="/" className="inline-block font-medium text-[#5DA865] hover:underline">← Back to Home</Link></div>
  </main>;
}
