...
type CategoryWithItems = { id: string; name: string; description: string | null; position: number; items: MenuItem[] };
...
export default function Tabs({ sections }: { sections: SectionWithCategories[] }) {
  ...
  return <>
    ...
    <div className="mx-auto max-w-3xl space-y-10">
      {active.categories.map((category) => <MenuSection key={category.id} title={category.name} description={category.description} items={category.items} />)}
    </div>
  </>;
}
