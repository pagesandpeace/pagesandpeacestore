    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mt-8 space-y-4">
        {orders.length ? orders.map((order) => {
          ...
          const netPaid = Math.max(0, Number(order.total_pence) - refunded);
          const refundStatus = order.refund_status ?? "none";
          const badge = refundStatus === "full"
          ...
          return <article key={order.id} className="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3">{order.lines.map((line) => {
              ...
              const fullyRefunded = refundedQty >= Number(line.quantity) && Number(line.quantity) > 0;
              ...
              return <li key={line.id} className="border-t pt-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    {refundedQty > 0 ? <p className="mt-1 text-sm font-medium text-red-700">{fullyRefunded ? "Refunded" : `${refundedQty} ticket${refundedQty === 1 ? "" : "s"} refunded`} · {money(refundedPence)}</p> : null}
                    {!fullyRefunded && refundedQty > 0 ? <p className="text-xs text-neutral-500">{remainingQty} ticket{remainingQty === 1 ? "" : "s"} remain active</p> : null}
                  </div>
                  {fullyRefunded ? <span className="rounded-full bg-red-100 px-2.5 py-1 text-xs font-semibold text-red-700">Refunded</span> : null}
                </div>
              </li>;
            })}</ul>

            <div className="mt-5 rounded-xl bg-[#F8F5F1] p-4 text-sm">
              <div className="flex justify-between gap-4"><span className="text-neutral-600">Original payment</span><strong>{money(Number(order.total_pence))}</strong></div>
