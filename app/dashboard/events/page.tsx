...
export default async function DashboardEventsPage() {
  ...
  const upcoming = orders.flatMap((order) => order.lines.map((line) => ({ line, orderId: order.id })))
    .filter(({ line }) => line.event && new Date(line.event.starts_at) >= now && Number(line.quantity) - Number(line.refunded_quantity ?? 0) > 0)
  ...
  return <main className="mx-auto max-w-4xl space-y-8 px-6 py-10">
    ...
    <section className="rounded-2xl border bg-white">
      ...
      {upcoming.length ? <div className="divide-y">{upcoming.map(({ line, orderId }) => {
        const remainingQty = Number(line.quantity) - Number(line.refunded_quantity ?? 0);
        return <article key={line.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div><h3 className="font-semibold">{line.event!.title}</h3><p className="mt-1 text-sm text-foreground/65">{formatDate(line.event!.starts_at)}</p><p className="mt-1 text-sm text-foreground/65">{line.ticket?.name ?? "Ticket"} × {remainingQty}</p></div>
      ...
      })}</div> : <div className="p-6 text-sm text-foreground/65">You have no upcoming bookings. <Link href="/events" className="underline">View upcoming events</Link>.</div>}
    </section>
