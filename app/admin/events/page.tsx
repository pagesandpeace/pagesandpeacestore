...
export default async function AdminEventsPage() {
  ...
  const { data: bookings } = eventIds.length
    ? await db.from("bookings").select("event_id, quantity, status, order_line_id").in("event_id", eventIds).in("status", ["pending", "confirmed"])
    : { data: [] };
  ...
  for (const booking of bookings ?? []) {
    const reserved = booking.status === "pending"
  ...
  return <div className="mx-auto max-w-6xl space-y-8 px-6 py-10">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">Events</h1></div><div className="flex gap-3"><Link href="/admin/events/bookings" className="rounded-lg border border-black/15 px-4 py-3 font-semibold">Bookings</Link><Link href="/admin/events/new" className="rounded-lg bg-black px-4 py-3 font-semibold text-white">Create event</Link></div></div>
