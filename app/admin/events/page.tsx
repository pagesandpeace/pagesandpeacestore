...
export default async function AdminEventsPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-8 px-6 py-10">
    ...
    <div className="overflow-hidden rounded-xl border border-black/10 bg-white"><table className="w-full text-left text-sm"><thead className="border-b bg-[#f8f5f1] text-foreground/70"><tr><th className="px-5 py-4 font-medium">Event</th><th className="px-5 py-4 font-medium">Starts</th><th className="px-5 py-4 font-medium">Capacity</th><th className="px-5 py-4 font-medium">Status</th><th className="px-5 py-4 font-medium">Actions</th></tr></thead><tbody className="divide-y">{(events ?? []).map((event) => { const reserved = reservedByEvent.get(event.id) ?? 0; const available = Math.max(event.capacity - reserved, 0); return <tr key={event.id}><td className="px-5 py-4"><p className="font-medium">{event.title}</p><p className="mt-1 text-xs text-foreground/55">/{event.slug}</p></td><td className="px-5 py-4 text-foreground/70">{formatDate(event.starts_at)}</td><td className="px-5 py-4"><p className="font-medium">{available} available</p><p className="text-xs text-foreground/60">{reserved} reserved · {event.capacity} total</p></td><td className="px-5 py-4"><span className="rounded-full bg-[#f2eee8] px-2.5 py-1 text-xs font-medium capitalize">{event.status}</span></td><td className="px-5 py-4"><Link href={`/admin/events/${event.id}/edit`} className="font-medium underline underline-offset-4">Edit</Link></td></tr>; })}</tbody></table>{!(events ?? []).length ? <p className="px-5 py-12 text-center text-foreground/60">No events yet.</p> : null}</div>
  </div>;
}
