...
export default async function AdminEventsPage() {
  ...
  const { data: events, error } = await db
    .from("events")
    .select("id, title, slug, starts_at, status, capacity, created_at")
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-6 py-10">
      ...
      <div className="overflow-hidden rounded-xl border border-black/10 bg-white">
          ...
          <tbody className="divide-y">
            {(events ?? []).map((event) => (
              <tr key={event.id}>
                ...
                <td className="px-5 py-4 text-foreground/70">{event.capacity}</td>
                <td className="px-5 py-4">
                  <span className="rounded-full bg-[#f2eee8] px-2.5 py-1 text-xs font-medium capitalize">
                    {event.status}
                  </span>
                </td>
              </tr>
        ...
        {(events ?? []).length === 0 ? (
          <p className="px-5 py-12 text-center text-foreground/60">No events yet.</p>
        ) : null}
      </div>
    </div>
  );
