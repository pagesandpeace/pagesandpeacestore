...
export default async function AdminOperationsClosedPage() {
  ...
  if (!user) redirect("/sign-in?callbackURL=/admin/operations/closed");
  ...
  if (error) {
    console.error("❌ CLOSED BACKORDERS LOAD FAILED", error);
  ...
  return (
    <div className="px-8 py-10 space-y-6">
      <h1 className="text-3xl font-semibold">Closed orders</h1>
          ...
          <TableBody>
            {rows.length === 0 ? (
              ...
              rows.map((b) => (
                <TableRow key={b.id}>
                  <Cell strong>
                    {b.title ?? resolveBackorderTitle(b)}
                  </Cell>
