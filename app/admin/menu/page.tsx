...
type Category = { id: string; name: string; position: number; website_area: WebsiteArea };
type Item = { id: string; category_id: string; name: string; price: number; position: number; note: string | null; is_visible: boolean };

const areaLabels: Record<WebsiteArea, string> = {
  drinks: "Menu: Drinks",
  food: "Menu: Food",
  snacks: "Menu: Snacks",
...
export default function AdminMenuPage() {
  ...
  const [newCategoryArea, setNewCategoryArea] = useState<WebsiteArea>("food");
  ...
  async function saveItem(item: Item) {
    ...
    const res = await fetch("/api/app-core/admin/menu", {
      ...
      body: JSON.stringify({ ...item, type: "item" }),
    });
    const data = await res.json().catch(() => null);
    setSavingId(null);
    setMessage(res.ok ? `${item.name} saved` : data?.error || "Could not save menu item");
  }
  ...
  async function saveCategory(category: Category) {
    ...
    const res = await fetch("/api/app-core/admin/menu", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "category", id: category.id, name: category.name, website_area: category.website_area }),
    });
    const data = await res.json().catch(() => null);
    setSavingId(null);
    setMessage(res.ok ? `${category.name} placement saved` : data?.error || "Could not save category");
  }
  ...
  async function addCategory() {
    ...
    const res = await fetch("/api/app-core/admin/menu", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "category", name: newCategory, website_area: newCategoryArea }),
    });
    const data = await res.json().catch(() => null);
    setCreating(null);
    if (!res.ok) return setMessage(data?.error || "Could not add category");
    setNewCategory("");
  ...
  async function addItem() {
    ...
    const res = await fetch("/api/app-core/admin/menu", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "item", category_id: newItem.category_id, name: newItem.name, price, note: newItem.note }),
    });
    const data = await res.json().catch(() => null);
    setCreating(null);
    if (!res.ok) return setMessage(data?.error || "Could not add menu item");
  ...
  return <main className="space-y-6">
    <div>
      ...
      <h1 className="mt-1 text-3xl font-bold">Menu & merchandise</h1>
      <p className="mt-2 text-foreground/65">Each category has a Website area. Drinks, Food and Snacks appear on /menu; Merchandise appears on /merch.</p>
    </div>
    ...
    <section className="grid gap-4 md:grid-cols-2">
      <div className="rounded-2xl border bg-white p-5">
        ...
        <div className="mt-4 grid gap-3">
          <input className="rounded-lg border px-3 py-2" placeholder="Category name" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} />
          <select className="rounded-lg border px-3 py-2" value={newCategoryArea} onChange={(e) => setNewCategoryArea(e.target.value as WebsiteArea)}>
            {Object.entries(areaLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
          </select>
          <button onClick={addCategory} disabled={creating === "category" || !newCategory.trim()} className="w-fit rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{creating === "category" ? "Adding…" : "Add category"}</button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-white p-5">
        ...
        <div className="mt-4 grid gap-3">
          <select className="rounded-lg border px-3 py-2" value={newItem.category_id} onChange={(e) => setNewItem((current) => ({ ...current, category_id: e.target.value }))}>
            ...
            {categories.map((category) => <option key={category.id} value={category.id}>{category.name} — {areaLabels[category.website_area]}</option>)}
          </select>
    ...
    <div className="space-y-6">
      {grouped.map((category) => <section key={category.id} className="rounded-2xl border bg-white p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <input className="rounded-lg border px-3 py-2 text-lg font-semibold" value={category.name} onChange={(e) => updateCategory(category.id, { name: e.target.value })} />
          <div className="flex gap-2">
            <select className="rounded-lg border px-3 py-2" value={category.website_area} onChange={(e) => updateCategory(category.id, { website_area: e.target.value as WebsiteArea })}>
              {Object.entries(areaLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
            </select>
            <button onClick={() => saveCategory(category)} disabled={savingId === category.id} className="rounded-full border border-accent px-4 py-2 text-sm font-semibold text-accent disabled:opacity-50">{savingId === category.id ? "Saving…" : "Save category"}</button>
          </div>
        </div>

        <div className="mt-4 space-y-3">
          {category.items.map((item) => <div key={item.id} className="grid gap-3 rounded-xl border p-4 md:grid-cols-[1.6fr_0.7fr_1.8fr_0.7fr_auto] md:items-center">
            ...
            <button onClick={() => saveItem(item)} disabled={savingId === item.id} className="rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{savingId === item.id ? "Saving…" : "Save"}</button>
          </div>)}
