...
export type CoreEvent = { id:string; slug:string; title:string; subtitle:string|null; short_description:string|null; description:string; starts_at:string; ends_at:string|null; capacity:number; image_url:string|null; series_id?:string|null; series_name?:string|null };
...
export async function listPublishedEvents(){const db=appCoreDb();const {data,error}=await db.from("events").select("id,slug,title,subtitle,short_description,starts_at,ends_at,capacity,image_url,series_id,series_name").eq("status","published").gte("starts_at",new Date().toISOString()).order("starts_at",{ascending:true});if(error)throw new Error("Unable to load events");return Promise.all((data??[]).map(async e=>({...e,remaining_seats:Math.max(e.capacity-await reservedSeats(e.id),0)})));}

export async function getPublishedEvent(slug:string){
  const db=appCoreDb();
  const {data:event,error}=await db.from("events").select("id,slug,legacy_slug,title,subtitle,short_description,description,starts_at,ends_at,capacity,image_url,series_id,series_name").or(`slug.eq.${slug},legacy_slug.eq.${slug}`).eq("status","published").maybeSingle();
