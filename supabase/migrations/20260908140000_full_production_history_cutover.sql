...
-- Resolve every genuine event line before writing. Old orders may have the ticket
-- type on the booking rather than the order item; one historical row is resolved
-- deterministically from its unique event-product title and matching price.
do $$
declare v_bad integer;
begin
  with real_event_items as (
    select oi.*
    from public.order_items oi
    join public.orders o on o.id = oi.order_id
    where oi.kind = 'event' and not coalesce(o.is_test, false)
  ), resolved as (
    select oi.id,
      coalesce(
        oi.event_ticket_type_id,
        (select min(b.event_ticket_type_id::text)::uuid
         from public.event_bookings b where b.order_item_id = oi.id),
        (select min(tt.id::text)::uuid
         from public.products p
         join public.events e on e.title = p.name
         join public.event_ticket_types tt
           on tt.event_id = e.id
          and tt.price_pence = round(oi.price * 100)::integer
         where p.id = oi.product_id)
      ) as ticket_type_id
    from real_event_items oi
  )
  select count(*) into v_bad from resolved where ticket_type_id is null;
  if v_bad <> 0 then
    raise exception 'HISTORY_CUTOVER_PRECHECK_FAILED: unresolved genuine event lines %', v_bad;
  end if;
end $$;
...
with referenced_events as (
  ...
  join public.events e on e.title = p.name
  where oi.kind = 'event' and oi.event_id is null and not coalesce(o.is_test,false)
), legacy as (
  select e.*,
    case
      when nullif(btrim(e.slug),'') is not null
       ...
       and not exists (
         select 1 from app_core.events ae
         where ae.slug = e.slug and ae.legacy_event_id is distinct from e.id
       )
      then e.slug
      else 'legacy-' || e.id::text
    end as cutover_slug
  from public.events e
  join referenced_events r on r.event_id = e.id
)
...
select o.id, o.user_id,
       ...
       round(o.total*100)::integer, 'gbp', o.stripe_checkout_session_id,
       o.stripe_payment_intent_id, o.stripe_receipt_url, o.stripe_card_brand,
       o.stripe_last4, o.customer_name, lower(o.customer_email), o.paid_at,
       o.created_at, coalesce(o.refund_status,'none'),
       round(coalesce(o.refunded_total,0)*100)::integer
...
select oi.id, oi.product_id, ao.id,
       ...
       coalesce(nullif(oi.name,''), p.name,
                case when oi.kind = 'event' then 'Event ticket' else 'Product' end),
       oi.quantity, round(oi.price*100)::integer, o.created_at,
       coalesce(oi.refunded_quantity,0),
       round(coalesce(oi.refunded_amount,0)*100)::integer
from resolved_items oi
join public.orders o on o.id = oi.order_id
join app_core.orders ao on ao.legacy_order_id = o.id
left join app_core.ticket_types att on att.legacy_ticket_type_id = oi.resolved_ticket_type_id
...
select b.id, aol.id, ae.id, att.id, u.auth_user_id, b.quantity,
       case
         when coalesce(b.refunded,false) then 'refunded'
         when coalesce(b.cancelled,false) then 'cancelled'
         when b.paid then 'confirmed'
         else 'pending'
       end,
       b.created_at, b.created_at, b.stripe_checkout_session_id,
       b.stripe_payment_intent_id, b.stripe_refund_id, b.refund_processed_at,
       b.name, lower(b.email)
from public.event_bookings b
join public.order_items oi on oi.id = b.order_item_id
...
set order_line_id = excluded.order_line_id,
...
select r.id, r.refund_scope, ao.id, aol.id,
       case when r.order_item_id is not null or r.event_booking_id is not null
            then 'order_line' else 'order' end,
       round(r.amount*100)::integer, r.reason, r.notes,
       r.stripe_payment_intent_id, r.stripe_refund_id,
       case when r.status='succeeded' then 'succeeded'
            when r.status='failed' then 'failed' else 'initiated' end,
...
left join app_core.order_lines aol
  on aol.legacy_order_item_id = coalesce(r.order_item_id,b.order_item_id)
...
set legacy_refund_scope = excluded.legacy_refund_scope,
    ...
    stripe_refund_id = excluded.stripe_refund_id,
    status = excluded.status,
    initiated_by_auth_user_id = excluded.initiated_by_auth_user_id,
...
declare
  ...
  s_qty bigint; t_qty bigint;
  s_total bigint; t_total bigint;
  s_ref bigint; t_ref bigint;
  s_gross bigint; t_gross bigint;
  s_lr bigint; t_lr bigint;
begin
  select count(*), coalesce(sum(round(total*100)::bigint),0),
         coalesce(sum(round(coalesce(refunded_total,0)*100)::bigint),0)
  into s_orders,s_total,s_ref
  ...
  into t_orders,t_total,t_ref
  from app_core.orders where legacy_order_id is not null;

  select count(*), coalesce(sum(round(price*quantity*100)::bigint),0),
         coalesce(sum(round(coalesce(refunded_amount,0)*100)::bigint),0)
  into s_lines,s_gross,s_lr
  from public.order_items oi
  join public.orders o on o.id=oi.order_id
  where not coalesce(o.is_test,false);

  select count(*), coalesce(sum(unit_amount_pence*quantity),0),
         coalesce(sum(refunded_amount_pence),0)
  into t_lines,t_gross,t_lr
  from app_core.order_lines where legacy_order_item_id is not null;

  select count(*), coalesce(sum(b.quantity),0)
  into s_bookings,s_qty
  from public.event_bookings b
  join public.order_items oi on oi.id=b.order_item_id
  join public.orders o on o.id=oi.order_id
  where not coalesce(o.is_test,false);
  ...
  if s_orders<>t_orders or s_lines<>t_lines or s_bookings<>t_bookings
     or s_qty<>t_qty or s_total<>t_total or s_ref<>t_ref
     or s_gross<>t_gross or s_lr<>t_lr then
    raise exception 'HISTORY_CUTOVER_RECONCILIATION_FAILED';
  end if;
end $$;
