...
export default function AdminSidebar({ user, profile }: { user: User; profile: UserProfile }) {
  ...
  return <aside className="fixed left-0 top-0 z-50 flex min-h-dvh w-64 flex-col justify-between border-r border-[#dcd6cf] bg-[#FAF6F1]">
    <div className="px-6 pt-10">
      ...
      <nav className="mt-8 flex flex-col gap-3 text-sm"><p className="text-xs uppercase tracking-wider text-gray-500">Admin</p>{nav.map((item) => <button key={item.href} onClick={() => router.push(item.href)} className="text-left font-medium hover:text-[#5DA865]">{item.label}</button>)}</nav>
    </div>
